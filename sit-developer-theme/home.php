<?php
/**
 * Bloq siyahısı (son yazılar səhifəsi).
 */
...
<main id="main-content" class="flex-1 py-12 lg:py-16">
	<div class="sit-container">
		<header class="mb-10 border-b border-slate-200 pb-8">
			<h1 class="text-3xl font-bold text-slate-900"><?php esc_html_e( 'Bloq', 'studyinturkey' ); ?></h1>
			<p class="mt-2 max-w-2xl text-slate-600"><?php esc_html_e( 'Təhsil, qəbul və Türkiyə haqqında yazılar.', 'studyinturkey' ); ?></p>
		</header>
		<?php if ( have_posts() ) : ?>
				...
				while ( have_posts() ) :
					...
					<article <?php post_class( 'group flex flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:border-brand-200 hover:shadow-md' ); ?>>
						...
						<div class="flex flex-1 flex-col p-5">
							<?php if ( ! empty( $cats ) ) : ?>
								<p class="text-xs font-medium uppercase tracking-wide text-brand-600"><?php echo esc_html( $cats[0]->name ); ?></p>
							<?php endif; ?>
		...
		<?php else : ?>
			<p class="text-slate-600"><?php esc_html_e( 'Hələ yazı yoxdur.', 'studyinturkey' ); ?></p>
		<?php endif; ?>
	</div>
</main>
<?php
