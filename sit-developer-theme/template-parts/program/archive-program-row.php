...
<tr class="border-b border-slate-200 bg-white transition-colors hover:bg-slate-50/80 dark:border-slate-700 dark:bg-slate-900 dark:hover:bg-slate-800/60">
	<td class="px-4 py-4 align-top">
		<div class="flex gap-3">
			<?php if ( $logo_url ) : ?>
				<a href="<?php echo esc_url( $univ_link ? $univ_link : $link ); ?>" class="h-10 w-10 shrink-0 overflow-hidden rounded-full border border-slate-100 bg-white p-0.5 dark:border-slate-600">
					<img src="<?php echo esc_url( $logo_url ); ?>" alt="" class="h-full w-full rounded-full object-contain" loading="lazy" width="40" height="40" />
				</a>
			<?php else : ?>
			...
			<div class="min-w-0">
				<?php if ( $univ_link ) : ?>
					<a href="<?php echo esc_url( $univ_link ); ?>" class="block truncate text-sm font-semibold text-slate-900 hover:text-brand-700 dark:text-white dark:hover:text-brand-300"><?php echo esc_html( $univ_title ); ?></a>
				<?php else : ?>
					<span class="text-sm font-semibold text-slate-500">—</span>
				<?php endif; ?>
				<?php if ( '' !== $field_sub ) : ?>
					<p class="mt-0.5 truncate text-xs text-slate-500 dark:text-slate-400"><?php echo esc_html( $field_sub ); ?></p>
				<?php endif; ?>
			</div>
		</div>
	</td>
	<td class="px-4 py-4 align-top">
		<a href="<?php echo esc_url( $link ); ?>" class="line-clamp-2 text-sm font-semibold text-slate-900 hover:text-brand-600 dark:text-white dark:hover:text-brand-400"><?php echo esc_html( $title ); ?></a>
	</td>
	...
	<td class="px-4 py-4 align-top">
		<?php if ( null !== $fee_num ) : ?>
			<div class="text-sm font-semibold whitespace-nowrap">
				...
				<?php endif; ?>
				<span class="ms-0.5 text-xs font-normal text-slate-500"><?php esc_html_e( '/ il', 'studyinturkey' ); ?></span>
			</div>
		<?php else : ?>
			<span class="text-slate-400">—</span>
		<?php endif; ?>
	...
	<td class="px-4 py-4 align-top text-end">
		<a href="<?php echo esc_url( $link ); ?>" class="inline-flex items-center justify-center rounded-lg bg-brand-600 px-3.5 py-2 text-xs font-semibold text-white shadow-sm whitespace-nowrap hover:bg-brand-700 dark:hover:bg-brand-500"><?php esc_html_e( 'Müraciət et', 'studyinturkey' ); ?></a>
	</td>
</tr>
