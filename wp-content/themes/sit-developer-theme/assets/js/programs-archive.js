	...
	function rowHtml(it) {
		...
		if (logoUrl) {
			logoBlock =
				'<a href="' +
				esc(uniHref) +
				'" class="h-9 w-9 shrink-0 overflow-hidden rounded-full border border-slate-100 bg-white p-0.5 dark:border-slate-600">' +
				'<img src="' +
				esc(logoUrl) +
				'" alt="" class="h-full w-full rounded-full object-contain" loading="lazy" width="36" height="36" />' +
				'</a>';
		} else {
		...
		var uniName =
			it.university_link && it.university_title
				? '<a href="' +
				  ...
				  '" class="block truncate text-xs font-semibold text-slate-900 hover:text-brand-700 dark:text-white dark:hover:text-brand-300">' +
				  esc(it.university_title) +
				  '</a>'
				: '<span class="text-xs font-semibold text-slate-500">—</span>';

		var fieldSub = fieldLine
			? '<p class="mt-0.5 truncate text-[11px] text-slate-500 dark:text-slate-400">' + esc(fieldLine) + '</p>'
			: '';
		...
		if (fee !== null && fee !== undefined && !isNaN(fee) && Number(fee) > 0) {
			...
			if (ref !== null && ref !== undefined && !isNaN(ref) && Number(ref) > Number(fee)) {
				feeBlock =
					...
					esc(main) +
					'$</span>' +
					'<span class="ms-1 text-[11px] font-normal text-slate-500">' +
					esc(cfg.strings.perYear) +
					'</span>' +
					'<br>' +
					'<span class="text-[11px] font-normal text-slate-400 line-through dark:text-slate-500">' +
					esc(fmtMoney(ref)) +
					'$</span></div>';
			} else {
				feeBlock =
		...
		return (
			...
			'<td class="px-3 py-3 align-top"><div class="flex gap-2.5">' +
			logoBlock +
			'<div class="min-w-0">' +
			uniName +
			fieldSub +
			'</div></div></td>' +
			'<td class="px-3 py-3 align-top"><a href="' +
			esc(it.link) +
			'" class="line-clamp-2 text-xs font-semibold text-slate-900 hover:text-brand-600 dark:text-white dark:hover:text-brand-400">' +
			esc(it.title) +
			'</a></td>' +
			...
			langCell +
			'</td>' +
			'<td class="px-3 py-3 align-top text-end"><a href="' +
			esc(it.link) +
			'" class="inline-flex items-center justify-center rounded-md bg-brand-600 px-3 py-1.5 text-[11px] font-semibold text-white shadow-sm whitespace-nowrap transition-colors hover:bg-brand-700 hover:text-white dark:hover:bg-brand-500">' +
